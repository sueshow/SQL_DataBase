...
* 類型
  * Key-Value資料庫
    * 特性：是 NoSQL 資料庫中最大宗的類型，具有水平擴充性、能依需求增加資料庫，類似字典
    * 用途：用於儲存 TB 或 PB 等級資料，如繪畫資料、配置設定
  ...
  * 文件資料庫 (Document Database)
    ...
    * 用途：用來儲存網頁資料 (JSON) 或各種 XML 格式的文件，也可儲存圖片或影音資料
    * 產品：CouchDB、MongoDB、Riak
  *  列族(Column Family)
    * 用於大型數據集的儲存和管理，如 Apache Cassandra，每個列族可包含多個列，每個列可包含多個欄位，進行高效的擴展
  * 圖學資料庫 (Graph Database)
    * 特性：資料包括節點 (Node)、關係 (Relation) 和屬性 (Property) 三種結構，來儲存圖學架構
    * 用途：運用圖學架構來儲存節點間關係資料架構，例如用樹狀結構來組織從屬關係或網狀結構來儲存朋友關係，地理圖資系統通常也會用圖學資料庫來儲存地圖上每一點和鄰近點的關係，或用圖學資料庫來計算點與點之間最短的距離，也可以用同樣的概念來計算出人與人之間最短的交友距離，如社交網路或知識圖譜等
    * 產品：Neo4j、InfoGrid、AllegroGrph
* 產品介紹
  * MongoDB：支援 JSON 格式的文件儲存和查詢，也可以處理二進位和數組數據
  * Cassandra：支援列族格式，可以處理結構化和非結構化資料，包括 JSON、XML 和文本等
  * Couchbase：支援 JSON 和二進位格式的文件，也可以處理鍵/值格式的數據
  * Redis：支援鍵/值格式和數組格式，並且可以進行高效的數據查詢和分析
  * Neo4j：支援圖格式，可以處理節點和邊上的結構化和非結構化資料
  * HBase：支援列族格式，可以處理結構化和非結構化資料，包括 XML、JSON 和二進位數據等
<br>
